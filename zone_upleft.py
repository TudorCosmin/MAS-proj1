...
async def scan_area(ctx: Context):    
    ...
    if matrix[i][j] == Guys.BAD_GUY:
        ...
        await ctx.send(environment.radar_upright.address, StopSearching())
        await ctx.send(environment.radar_bottomleft.address, StopSearching())
        await ctx.send(environment.radar_bottomright.address, StopSearching())
    else:
...
@radar_upleft.on_message(model=StopSearching, replies=set())
async def give_help(ctx: Context, sender: str, msg: StopSearching):
    ctx.storage.set("moving", False)
    ctx.logger.info("radar top left stopped-----------------------------")