...
async def scan_area(ctx: Context):
    ...
    if matrix[i][j] == Guys.BAD_GUY:
        ...
        await ctx.send(radar_bottomright.address, StopSearching())
    else:
...
@radar_bottomleft.on_message(model=StopSearching, replies=set())
async def give_help(ctx: Context, sender: str, msg: StopSearching):
    ctx.storage.set("moving", False)
    ctx.logger.info("radar bottom left stopped-----------------------------")